...
def render_i2v_seedance(ark_client):
    ...
    with col1:
        ...
        if byteplus_enabled:
            # 从设置中获取 Byteplus 图生视频模型
            i2v_models_str = st.session_state.get("byteplus_i2v_model", "seedance-1-0-pro-250528,seedance-1-0-lite-i2v-250428")
            # 拆分逗号分隔的模型字符串为数组
            model_options = [model.strip() for model in i2v_models_str.split(",") if model.strip()]
        else:
            # 使用默认的 Volcengine 模型
            model_options = ["doubao-seedance-1-0-pro-250528", "doubao-seedance-1-0-lite-i2v-250428"]
